...
array.sort(function (e1, e2) { return e2 - e1; });
...
function getLast(array, itemsNumber) {
    return array.slice(array.length - itemsNumber, array.length);
}
console.log(`Последние 5 элементов: ${getLast(array, 5).join(", ")}`);

function getEvens(array) {
    return array.filter(function (n) { return !(n % 2); });
}
...
function getArraySum(array) {
    return array.reduce(function (result, currentItem) { return result + currentItem });
}
...
function powArrayElements(array, pow) {
    return array.map(function (n) { return Math.pow(n, pow); });
}
